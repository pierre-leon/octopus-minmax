## v1.0.17-PL - Fix start-enrolment payload
Fixes the 422 that stopped the first real switch attempt on v1.0.16-PL.

Octopus's `start-enrolment` endpoint wants `propertyId` as a number and answers a quoted one with `422 {"errors":[{"message":"Invalid body"}]}`. The value is now coerced where `enrolment-data` is read, with a readable error if Octopus ever returns something non-numeric, and again when the request body is built so the type cannot drift back. The payload is byte-for-byte identical to the one the website sends.

Everything else in that attempt already worked: the pasted session authenticated, eligibility passed, and the product-code guard confirmed Octopus was offering the same product the comparison chose.
