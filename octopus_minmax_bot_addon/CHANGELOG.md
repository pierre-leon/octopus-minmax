## v1.0.12-PL - v1.0.12-PL
Optional comparison chart (on by default), hatching for negative consumption, and labels kept inside the chart frame.

Use SEND_COMPARISON_CHART=false (or uncheck it in HA / the dashboard) for text-only notifications.
