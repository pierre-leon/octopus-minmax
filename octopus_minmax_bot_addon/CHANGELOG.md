## v1.0.13-PL - Customer login for tariff switching
## Summary
- Octopus API keys can still compare tariffs but are no longer allowed to start a switch (`startOnboardingProcess` returns Unauthorized).
- Sign in once via the dashboard **Octopus Login** page. The bot stores a refresh token (not your password) and reuses it on later runs.
- Set `DASHBOARD_URL` so login notifications point at a reachable host, and persist `./data` (Docker) or `/data` (Home Assistant).

## Tested
Email/password login against live Octopus succeeded and returned a customer refresh token.

## Upgrade notes
This image is not the store add-on yet. Run this tag (or this branch) and complete Octopus Login before the first real switch.
