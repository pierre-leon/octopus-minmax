## v1.0.14-PL - OAuth login for tariff switching
## Summary
- Dashboard **Octopus Login** now uses the public GraphQL IDE OAuth client (authorization code + PKCE, `full-customer-access`) instead of GraphQL email/password.
- After Octopus redirects to the GraphQL IDE, paste the URL `code=` or the `refresh_token` back into the bot. Access tokens expire in 1 hour; the bot refreshes on each comparison run.
- API-key auth still compares tariffs and can accept T&Cs on an enrolment the website already started. `startOnboardingProcess` with this OAuth grant is untested until the next real switch.

## Upgrade notes
This image is not the store add-on. Run this tag (or `octopus-login-refresh`), disconnect any old email/password session, and complete OAuth login once. Persist `./data` (Docker) or `/data` (Home Assistant).
