        ...
        for entry in result['productEnrolments']:
            for stage in entry['stages']:
                if stage['name'] == 'post-enrolment':
                    last_step_date = datetime.fromisoformat(stage['steps'][-1]['updatedAt'].replace('Z', '+00:00')).date()
...
def get_acc_info():
    ...
    result = gql_client.execute(gql(consumption_query.format(device_id = device_id, start_date = f"{date.today()}T00:30:00Z", end_date = f"{date.today()}T23:59:59Z" )))
...
def get_potential_tariff_rates(tariff, region_code):
    ...
    today = date.today()
    unit_rates = rest_query(f"{config.BASE_URL}/products/{tariff_code}/electricity-tariffs/{product_code}/standard-unit-rates/?period_from={today}T00:00:00Z&period_to={today}T23:59:59Z")
    standing_charge = rest_query(f"{config.BASE_URL}/products/{tariff_code}/electricity-tariffs/{product_code}/standing-charges/")
...
def compare_and_switch():
    ...
    summary = "Total potential cost: £{:.2f} vs current cost: £{:.2f}".format(total_potential_calculated / 100, total_curr_cost / 100)
    ...
    if (total_potential_calculated + 2) < total_curr_cost:
        ...
        send_discord_message("Tariff swtich requested successfuly.")
...
def main():
    try:
        setup_gql(get_token())
        if gql_transport != None and gql_client != None:
            compare_and_switch()
        else:
